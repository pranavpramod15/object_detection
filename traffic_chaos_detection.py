...
def process_frame(model, frame, vehicle_tracks, turn_counts, turns_by_time, entries_by_time, current_time_sec):
    turning_bb_color = {
        'Right': (0, 0, 255),
        'Left': (0, 255, 0),
        'U-turn': (0, 0, 0),
    ...
    for idx, turn in enumerate(['Right', 'Left', 'U-turn']):
        cv2.putText(frame, f"{turn} Turns: {turn_counts[turn]}", (30, 30 + 30 * idx),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, turning_bb_color[turn], 2)
...
def run_traffic_chaos_detection(video_path: str, result_json_path: str, output_video_path: str):
    ...
    output = cv2.VideoWriter(output_video_path, cv2.VideoWriter_fourcc(*'avc1'), fps, (width, height))
    ...
    while True:
        ...
        frame = process_frame(model, frame, vehicle_tracks, turn_counts, turns_by_time, entries_by_time, current_time_sec)
        output.write(frame)
